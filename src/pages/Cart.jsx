                ...
                <div className="space-y-6">
                    <AnimatePresence>
                        {cart.map((item) => (
                            <motion.div
                                key={item.id}
                                className="flex flex-col sm:flex-row gap-6 p-6 bg-white border border-gray-100 rounded-xl shadow-sm hover:shadow-md transition-shadow group"
                                layout
                                initial={{ opacity: 0, x: -20 }}
                                animate={{ opacity: 1, x: 0 }}
                                exit={{ opacity: 0, scale: 0.9 }}
                            >
                                <div className="w-full sm:w-32 h-40 bg-gray-50 p-4 rounded-lg flex-shrink-0">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-4 bg-gray-50 px-3 py-1.5 rounded-full border border-gray-100">
                                                <button
                                                    ...
                                                    className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-white transition-colors disabled:opacity-30"
                                                >
                                                    <Minus size={14} />
                                                </button>
                                            ...
                                            <div className="flex items-center gap-4">
                                                <button
                                                    ...
                                                    className={`p-2 rounded-full hover:bg-red-50 transition-colors ${isInWishlist(item.id) ? 'text-accent' : 'text-gray-300'}`}
                                                >
                    ...
                    <Link to="/shop" className="inline-flex items-center gap-2 font-bold text-sm hover:text-accent transition-colors group">
                        <ArrowLeft size={18} className="transition-transform group-hover:-translate-x-1" /> Continue Shopping
                    </Link>
                </div>

                <aside className="self-start sticky top-28 bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
                    ...
                    <button
                        ...
                        className="w-full bg-accent text-white py-4 font-bold rounded-xl shadow-lg hover:bg-red-600 shadow-accent/20 transition-all transform hover:-translate-y-1 uppercase tracking-wider mb-6"
                    >
                        PROCEED TO CHECKOUT
                    </button>
