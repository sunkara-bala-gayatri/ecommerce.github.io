...
const Checkout = () => {
    ...
    if (isOrdered) {
        return (
            <div className="container py-20 md:py-32 flex flex-col items-center justify-center text-center px-4">
                <motion.div
                    initial={{ scale: 0.8, opacity: 0 }}
                    animate={{ scale: 1, opacity: 1 }}
                    className="bg-white p-8 md:p-12 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.05)] border border-gray-100 max-w-xl w-full flex flex-col items-center gap-6"
                >
                    <div className="w-20 h-20 md:w-24 md:h-24 bg-green-50 rounded-full flex items-center justify-center text-green-500 mb-2">
                        <CheckCircle size={48} md:size={64} strokeWidth={1.5} />
                    </div>
                    <h1 className="text-2xl md:text-3xl font-bold font-serif uppercase tracking-tight leading-tight">Order Placed Successfully!</h1>
                    <p className="text-sm md:text-base text-text-muted leading-relaxed">Thank you for shopping with Fashion Hall. Your order <span className="text-primary font-bold">#FH{Math.floor(Math.random() * 100000)}</span> is being processed and will be delivered within 3-5 business days.</p>
                    <Link to="/" className="w-full bg-primary text-white py-4 font-bold rounded-xl shadow-lg hover:bg-accent transition-all transform active:scale-95 uppercase tracking-widest text-sm mt-4">
                        CONTINUE SHOPPING
                    </Link>
                </motion.div>
            </div>
                ...
                <form className="space-y-8" onSubmit={handlePlaceOrder}>
                    <div className="bg-white p-6 md:p-8 rounded-2xl border border-gray-100 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                ...
                                <input type="text" placeholder="Jane" className="w-full bg-gray-50 border border-transparent px-4 py-3.5 rounded-xl focus:bg-white focus:border-accent outline-none transition-all text-sm font-medium" required value={formData.firstName} onChange={(e) => setFormData({...formData, firstName: e.target.value})} />
                            </div>
                            <div className="space-y-2">
                                <label className="text-[11px] font-bold text-text-muted uppercase ml-1">Last Name</label>
                                <input type="text" placeholder="Doe" className="w-full bg-gray-50 border border-transparent px-4 py-3.5 rounded-xl focus:bg-white focus:border-accent outline-none transition-all text-sm font-medium" required value={formData.lastName} onChange={(e) => setFormData({...formData, lastName: e.target.value})} />
                            </div>
                            <div className="md:col-span-2 space-y-2">
                                <label className="text-[11px] font-bold text-text-muted uppercase ml-1">Email Address</label>
                                <input type="email" placeholder="jane@example.com" className="w-full bg-gray-50 border border-transparent px-4 py-3.5 rounded-xl focus:bg-white focus:border-accent outline-none transition-all text-sm font-medium" required value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} />
                            </div>
                            <div className="md:col-span-2 space-y-2 relative">
                                ...
                                <input 
                                    ...
                                    className="w-full bg-gray-50 border border-transparent px-4 py-3.5 rounded-xl focus:bg-white focus:border-accent outline-none transition-all text-sm font-medium" 
                                    required 
                                    ...
                                    <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-gray-100 rounded-xl shadow-2xl z-20 overflow-hidden">
                                        {addressSuggestions.map((addr, i) => (
                                            <button
                                                ...
                                                className="w-full px-4 py-4 text-left text-sm hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0 font-medium"
                                            >
                                                {addr.address}
                                            </button>
                                        ))}
                            ...
                            <div className="space-y-2">
                                ...
                                <input type="text" placeholder="New York" className="w-full bg-gray-50 border border-transparent px-4 py-3.5 rounded-xl focus:bg-white focus:border-accent outline-none transition-all text-sm font-medium" required value={formData.city} onChange={(e) => setFormData({...formData, city: e.target.value})} />
                            </div>
                            <div className="space-y-2">
                                <label className="text-[11px] font-bold text-text-muted uppercase ml-1">Postal Code</label>
                                <input type="text" placeholder="10001" className="w-full bg-gray-50 border border-transparent px-4 py-3.5 rounded-xl focus:bg-white focus:border-accent outline-none transition-all text-sm font-medium" required value={formData.zip} onChange={(e) => setFormData({...formData, zip: e.target.value})} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-6 md:p-8 rounded-2xl border border-gray-100 shadow-sm">
                        ...
                        <div className="space-y-4 mb-8">
                            <label className="flex items-center justify-between p-5 border-2 border-accent bg-red-50/30 rounded-2xl cursor-pointer transition-all group">
                        ...
                        <div className="space-y-6">
                            <div className="space-y-2">
                                ...
                                <input type="text" placeholder="•••• •••• •••• ••••" className="w-full bg-gray-50 border border-transparent px-4 py-3.5 rounded-xl focus:bg-white focus:border-accent outline-none transition-all text-sm font-medium" />
                            </div>
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[11px] font-bold text-text-muted uppercase ml-1">Expiry Date</label>
                                    <input type="text" placeholder="MM / YY" className="w-full bg-gray-50 border border-transparent px-4 py-3.5 rounded-xl focus:bg-white focus:border-accent outline-none transition-all text-sm font-medium" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[11px] font-bold text-text-muted uppercase ml-1">CVV</label>
                                    <input type="password" placeholder="•••" className="w-full bg-gray-50 border border-transparent px-4 py-3.5 rounded-xl focus:bg-white focus:border-accent outline-none transition-all text-sm font-medium" />
                                </div>
                            </div>
                        </div>
                    </div>

                    <button type="submit" className="w-full bg-primary text-white py-5 font-bold rounded-2xl shadow-xl hover:bg-accent transition-all transform active:scale-95 uppercase tracking-[3px] text-sm flex items-center justify-center gap-3">
                        PLACE ORDER <CheckCircle size={20} />
                    </button>

                    <p className="text-center text-[10px] md:text-[11px] text-text-muted uppercase tracking-widest flex items-center justify-center gap-2">
                        <Lock size={12} className="text-green-500" /> Secure encrypted checkout
                    </p>
                </form>

                <aside className="self-start lg:sticky lg:top-28 space-y-8">
                            ...
                            {cart.map(item => (
                                <div key={item.id} className="flex gap-4 items-center">
                                    <div className="w-14 h-16 bg-white/10 p-2 rounded-lg flex-shrink-0">
                                        <img src={item.image} alt={item.title} loading="lazy" className="w-full h-full object-contain filter brightness-110" />
                                    </div>
